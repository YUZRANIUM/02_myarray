...
^
* ary1, ary2 ともに、1次元配列でなければなりません。多次元配列の場合はcnvarray命令で1次元配列に変換して下さい。
* p1 に0を指定した場合はary1が昇順、1を指定した場合は降順にソートされます。
^
    p1 |      動  作       |          例
--------------------------------------------------------
    0  |  昇順 (小さい順)  | ..., -2, -1, 0, 1, 2, ...
...
探索に成功した場合は一致した名称を文字列で返します。探索に失敗した場合は、ラベル型: <null-label>, モジュール型: [not-found] がそれぞれ返ります。
%sample
