...
^
^
※この命令は配列操作のみを行っており、実際の数値計算はcalc_var_ary_(local)関数で行わせています。比較的簡単な作りとなっており、計算式を追加していくことでカスタムすることが可能です。02_myarray.hspのEOF近くにあります。
...
^
calc_ary命令は呼び出されるたびに第1パラメータに指定された変数名を初期化します。この命令はその初期化を防ぐフラグを立てます。
...
^
dimlinec関数を利用することで、全ての多次元配列を1次元配列のように扱うことが可能になります。ctlarray命令では "1次元化要素数" で開始値と終了値を指定することになります。ctlarray命令で多次元配列を扱う場合はdimlinec関数を使用して下さい。
^
...
srch : 結果を受け取る変数名
%inst
この命令は、itemで指定された値をlistの中で二分木探索を行い、listの配列要素をsrchに読み出すものです。
^
* list はあらかじめ、昇順ソートされていることが前提です。
^
実行後、探索に成功した場合はlistの配列要素がsrchに代入されます。探索に失敗した場合はsrchに-1が代入されます。
...
^
    b(3, 2) = 0
^
上記のように、表示させたい変数名とその値、そして配列要素を管理している変数が保持している値に置換されて表示されます。これは、配列操作命令のpriarray, labarrayと同じく、#defineの%1,%2,%3,...がダブルクォーテーションで囲まれた状態でも展開されるという性質を利用しています。
^
...
b(3, 2) = 0
^
上記のように、エディターでlogpri命令を呼び出した行数と、表示させたい変数名とその値、そして配列要素を管理している変数が保持している値に置換されて表示されます。これは、配列操作命令のpriarray, labarrayと同じく、#defineの%1,%2,%3,...がダブルクォーテーションで囲まれた状態でも展開されるという性質を利用しています。
^
