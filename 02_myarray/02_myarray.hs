...
var1 : 各種情報が読み出される変数名
%inst
この命令は、ary1に指定された配列変数に関する各種情報をvar1に読み出すものです。
^
この命令の実行後、var1には以下の内容が代入されます。
^
        var1の内容
------------------------
    var1(0) : length(ary1)
    ...
    var1(4) : ary1の使用次元数 (1, 2, 3, 4)
    var1(5) : ary1の配列の長さ (総数)
    var1(6) : ary1の変数型 (1, 2, 3, 4, 5, 6)
^
var1(4)の使用次元数とは、2次元配列変数の2、3次元配列変数の3、4次元配列変数の4と言った具合に名称で用いられる値です。
var1(5)の配列の長さとは、その変数の各次元要素数の総数を表します。
var1(6)の変数型はvartypeと同様の値です。
^
注意事項としては、情報取得の対象となる配列変数の初期化直後に本命令を使用して下さい。
...
d1,d2,d3,d4 : 各次元要素 (= 0)
%inst
この関数は、ary1に指定された多次元配列の要素(d1, d2, d3, d4)を、1次元配列に変換した際の要素（1次元化要素数）を返すものです。
^
...
^
例えば、dim a, 3, 3, 2 と初期化した3次元配列変数aは、1次元配列変数zで下のように表すことができます。
^
        a(0, 0, 0)   ==>   z(0)
        a(1, 0, 0)   ==>   z(1)
        a(2, 0, 0)   ==>   z(2)
        ...
        a(2, 2, 1)   ==>   z(17)
^
上記のように、多次元配列変数の要素数を1次元で表す操作のことを他言語では "1次元化" または "平坦化"、"フラット化"、 などと呼ぶことがあります。
^
このとき、1次元配列変数zの要素は3次元配列変数aの先頭a(0, 0, 0)を基点としたオフセット値と捉えることができ、3次元配列変数aの各次元の要素の組み合わせに対して一対一の関係にあります。
この関係は3次元配列変数aの次元数、または配列の長さが変化しない限り保持されます。

そのため、1次元配列変数zの要素を、"aのオフセット値" または "aの1次元化要素数" と呼んでいます。
特に、"オフセット値" (1次元化要素数) の総数を "配列の長さ" または 単に "長さ" と呼ぶことがあります。
例えば、1次元配列変数zの要素数の総数は z(0)からz(17)までの18なので「3次元配列変数aの "長さ" は 18 である」と表現できます。
^
^
d1以降をすべて省略しdimlinec(a)とした場合は "配列の長さ" つまり上の例では 18 が返り、一方で、d1以降を指定した場合、例えば上記a(0, 1, 1)のときz(12)ですので、dimlinec(a, 0, 1, 1)とした場合は 12 が返ります。
^
^
このように、"配列のオフセット値" (1次元化要素数) を返す関数がdimlinec関数です。
^
dimlinec関数を利用することで、多次元配列を1次元配列のように扱うことが可能になります。
ctlarray命令やMDAQSort命令では "1次元化要素数" で開始値と終了値を指定することになりますので、これらの命令で多次元配列変数を扱う場合はdimlinec関数を使用して下さい。
^
...
^
で初期化された状態となるのでary1と同じ変数型、次元要素数の配列となります。ただし、演算タイプが9(べき乗)の場合はans_aryを実数型で初期化します。ary2の変数型や次元要素数はans_aryに何らの影響はありません。これは、HSPの言語仕様にならっての仕様です。
^
またこの仕様によって、総計算回数はary1の配列の長さになります。ary1よりも、ary2の方が配列として短い場合は短い分だけary2が周回し、ary2の方が長い場合はary1の長さで計算を終えます。
^
